...
-- Effect: +15% [ICON_PRODUCTION] Production toward Wonders for any Civilization that owns this tile.
-----------------------------------------------

UPDATE	Features
SET		Description = 'LOC_TM_FEATURE_IK_KIL_DESCRIPTION'
WHERE	FeatureType = 'FEATURE_IK_KIL';
	...
	WHERE EXISTS (SELECT * FROM Features WHERE FeatureType = 'FEATURE_IK_KIL')
	AND EXISTS (SELECT * FROM TM_UserSettings WHERE Setting = 'NW_EFFECTS' AND Value = 1);
...
VALUES	('MODIFIER_TM_FEATURE_IK_KIL_ATTACH_PLAYERS',		'MODTYPE_TM_ATTACH_ALL_PLAYERS',		'REQSET_TM_PLAYER_HAS_FEATURE_IK_KIL'	),
